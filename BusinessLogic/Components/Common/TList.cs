...
namespace Interact.BusinessLogic
{
	...
	public class TList<T> : BindingList<T>, IBindingListView, IBindingList, IList, ICloneable, IListSource, ITypedList, IDisposable, IComponent, IRaiseItemChangedEvents, IDeserializationCallback where T : IEntity, new()
      ...
      public void ApplyFilter(Predicate<T> match)
      {
         ...
         this.excludedItems.Clear();
      	...
      	foreach(T item in this.Items)
      	{
      		if (!match(item))
      		{
      			iList.Remove(item); // we do this on IList so we're not firing events
      ...
      #region Find
		...
		{
			return Find(column.ToString(), value);
		}
		...
		{
			return FindAll(column.ToString(), value);
		}
      ...
      public void OnDeserialization(object sender)
      {
		...
		/// </returns>
		public static bool IsNullOrEmpty(TList<T> list)
		{
		...
		/// </summary>
		/// <param name="array">The array.</param>
		public void AddRange(TList<T> list)
		{
		...
		/// <summary>
		/// Searches for an element that matches the conditions defined by the specified predicate, and returns the first occurrence within the entire TList.
		/// </summary>
		/// <param name="match">The <c>Predicate</c> delegate that defines the conditions of the element to search for.</param>
